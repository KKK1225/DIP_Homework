...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask, background_img):
    ...
    # bg_img = (background_img * background_mask.repeat(1,3,1,1))[:, :, min_bg_x:max_bg_x+1, min_bg_y:max_bg_y+1]
    # bl_img = (blended_img * background_mask.repeat(1,3,1,1))[:, :, min_bg_x:max_bg_x+1, min_bg_y:max_bg_y+1]

    fg_img = foreground_img[:, :, min_fg_x:max_fg_x+1, min_fg_y:max_fg_y+1]
    bg_img = background_img[:, :, min_bg_x:max_bg_x+1, min_bg_y:max_bg_y+1]
    bl_img = blended_img[:, :, min_bg_x:max_bg_x+1, min_bg_y:max_bg_y+1]
    ...
    bl_result = F.conv2d( bl_img, laplacian_kernel_1, padding=1)
    loss_1 = (fg_result - torch.where(torch.abs(bg_result) >= torch.abs(bl_result), bg_result, bl_result)) ** 2

    fg_result = F.conv2d( fg_img, laplacian_kernel_2, padding=1)
    bg_result = F.conv2d( bg_img, laplacian_kernel_2, padding=1)
    bl_result = F.conv2d( bl_img, laplacian_kernel_2, padding=1)
    loss_1 = (fg_result - torch.where(torch.abs(bg_result) >= torch.abs(bl_result), bg_result, bl_result)) ** 2 + loss_1

    fg_result = F.conv2d( fg_img, laplacian_kernel_3, padding=1)
    bg_result = F.conv2d( bg_img, laplacian_kernel_3, padding=1)
    bl_result = F.conv2d( bl_img, laplacian_kernel_3, padding=1)
    loss_1 = (fg_result - torch.where(torch.abs(bg_result) >= torch.abs(bl_result), bg_result, bl_result)) ** 2 + loss_1

    fg_result = F.conv2d( fg_img, laplacian_kernel_4, padding=1)
    bg_result = F.conv2d( bg_img, laplacian_kernel_4, padding=1)
    bl_result = F.conv2d( bl_img, laplacian_kernel_4, padding=1)
    loss_1 = (fg_result - torch.where(torch.abs(bg_result) >= torch.abs(bl_result), bg_result, bl_result)) ** 2 + loss_1
